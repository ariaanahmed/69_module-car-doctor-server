...
async function run() {
  try {
    ...
    const serviceCollection = client.db('carDoctorsServices').collection('services');
    const bookingCollection = client.db('carDoctorsServices').collection('bookings')
...
app.get('/', (req, res) => {
  res.send('CAR DOCTOR IN RUNNING');
})
